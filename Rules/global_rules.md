...
4. **선 계획 후 실행 원칙 (Plan Before Action):** 어떠한 파일 수정(Writing/Modifying), 생성, 삭제 또는 상태를 변화시키는 쉘 명령어(예: git 관련 명령어 등)를 실행하기 전에, 반드시 변경 목적과 설계안을 먼저 브리핑하고 사용자의 명시적인 승인(verbal/written consent)을 얻어야 합니다. 사용자가 동의하기 전까지는 임의로 수정이나 실행을 시작하지 마십시오. 복잡한 작업이나 구조적 변경의 경우 `implementation_plan.md` 작성을 통한 승인 프로세스를 필수로 선행해야 합니다.
